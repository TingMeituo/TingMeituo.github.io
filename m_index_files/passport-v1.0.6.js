...
(function($) {
	 window.PassPortUtil = {
			  ...
			  *  @param clientHostLogin passport应用的回调登录地址
			  */
			 loginPhone:function(appId,phonenum,code,cb,cbUrl,passportHost,clientHostLogin) {
				 $.ajax({  
				        url:passportHost+'/login_jsonp_phonecode.do?jsonpCallback=?',  
				        dataType:'jsonp',  
				        data:{appId:appId,phonenum:phonenum,code:code,service:cbUrl,appLogin:clientHostLogin},  
				        jsonp:'callback',  
			 ...
			 logout:function(appLogoutUrl,appId,passportHost,isRedirect,cbUrl) {
				 ...
				 $.when($z.logoutAppOne(passportHost),$z.logoutAppThree(passportHost),$z.logoutAppGogoup(passportHost),$z.logoutProHellorf(passportHost),$z.logoutBlogHellorf(passportHost),$z.logoutHellorfPre(passportHost), $z.logoutZDo(passportHost)).done($z.logoutServer(str)).fail(function(){
					 ...
					 $z.logoutZDo(passportHost);
					 $z.logoutServer(str);
				 });
			 },
			 logoutAppOne:function(passportHost)  {
				 if(passportHost != null && passportHost.indexOf('dev') > -1){
					 $z.addScriptTag('http://www.dev.zcool.com.cn/sso/logout_jsonp.do?callback=$z.logoutCallBack&scriptId=ssoscript10061','ssoscript10061');
					 $z.addScriptTag('http://my.2017.dev.zcool.com.cn/logout?callback=$z.logoutCallBack&scriptId=ssoscript10171','ssoscript10171');
				 }else if(passportHost != null && passportHost.indexOf('test') > -1){
                     $z.addScriptTag('http://my.2017.test.zcool.com.cn/logout?callback=$z.logoutCallBack&scriptId=ssoscript10172','ssoscript10172');
                     ...
                     $z.addScriptTag('http://my.2017.zcool.com.cn/logout?callback=$z.logoutCallBack&scriptId=ssoscript1017','ssoscript1017');
...
//				 }
			 ...
			 logoutZDo:function(passportHost) {
				 if(passportHost != null && passportHost.indexOf('test') > -1){
					 $z.addScriptTag("http://test.zdo.zcool.com.cn/logout.do?callback=$z.logoutCallBack&scriptId=ssoscript10152",'ssoscript10152');
				 }else if(passportHost != null && passportHost.indexOf('dev') > -1){
					 $z.addScriptTag("http://dev.zdo.zcool.com.cn/logout.do?callback=$z.logoutCallBack&scriptId=ssoscript10151",'ssoscript10151');
				 }else{
					 $z.addScriptTag("http://zdo.zcool.com.cn/logout.do?callback=$z.logoutCallBack&scriptId=ssoscript1015",'ssoscript1015');
				 }
			 },
...
//				 console.log('logoutCallback:'+flag);
			 ...
			 */
			 ...
			 repeatLoginLogout:function(currentAppLogoutUrl,appId,passportHost,isRedirect,cbUrl) {
				 $.when($z.logoutAppOne(passportHost),$z.logoutAppThree(passportHost),$z.logoutAppGogoup(passportHost),$z.logoutProHellorf(passportHost),$z.logoutBlogHellorf(passportHost),$z.logoutHellorfPre(passportHost), $z.logoutZDo(passportHost))
				 .done(function(){
					 if(cbUrl) {
...
//					 console.log('重复登录,退出失败!');
			 ...
			 repeatLoginLogoutCb:function(currentAppLogoutUrl,appId,passportHost,isRedirect,data,callback) {
				 $.when($z.logoutAppOne(passportHost),$z.logoutAppThree(passportHost),$z.logoutAppGogoup(passportHost),$z.logoutProHellorf(passportHost),$z.logoutBlogHellorf(passportHost),$z.logoutHellorfPre(passportHost), $z.logoutZDo(passportHost))
				 .done(function(){
					 callback(data)
				 })
...
//					 console.log('重复登录,退出失败!');
			...
			/* addImgTag:function(src) {
			 ...
			 encryptFn:function(appId,userid,password,remember,code,cb,cbUrl,passportHost,clientHostLogin){
				 $.ajax({  
				        url:passportHost+'/passKey.do?jsonpCallback=?',  
				        dataType:'jsonp',  
				        success:function(data) {
				        	if(data != null){
				        		setMaxDigits(130);
								...
								$.ajax({  
							        url:passportHost+'/login_jsonp_active.do?jsonpCallback=?',  
							        dataType:'jsonp',  
